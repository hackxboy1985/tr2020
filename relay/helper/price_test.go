...
func TestModelPriceHelperTieredV2UsesPerCallQuotaConversion(t *testing.T) {
	...
	require.NoError(t, config.GlobalConfig.LoadFromDB(map[string]string{
		"billing_setting.billing_mode": `{"gemini-image-test-model":"tiered_expr"}`,
		"billing_setting.billing_expr": `{"gemini-image-test-model":"v2: tier(\"2K\", param(\"generationConfig.imageConfig.imageSize\") == \"2K\" ? 0.04 : 0.02)"}`,
	}))

	recorder := httptest.NewRecorder()
	ctx, _ := gin.CreateTestContext(recorder)
	req := httptest.NewRequest(http.MethodPost, "/v1beta/models/gemini-image-test-model:generateContent", nil)
	req.Body = nil
	req.ContentLength = 0
	req.Header.Set("Content-Type", "application/json")
	ctx.Request = req
	...
	info := &relaycommon.RelayInfo{
		OriginModelName: "gemini-image-test-model",
		UserGroup:       "default",
		UsingGroup:      "default",
		RequestHeaders:  map[string]string{"Content-Type": "application/json"},
		BillingRequestInput: &billingexpr.RequestInput{
